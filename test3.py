...
X_train = metamorphic_feature_rotation_with_angle(X_train, 0.1)

X_test = metamorphic_feature_rotation_with_angle(X_test, 0.1)
...
def __getAngleEmdedding(a, b):
    ...
    qml.AngleEmbedding(a, wires=range(8)) 
    qml.adjoint(qml.AngleEmbedding(b, wires=range(8))) 
    return qml.probs(wires = range(8))

# Define the kernel matrix function
def kernel_matrix(X1, X2):
    return np.array([[__getAngleEmdedding(x1, x2) for x2 in X2] for x1 in X1])
